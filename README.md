...
| `tools/state.py` | the run manifest: checkpoint every move, resume after a crash. A move interrupted mid-flight is left WIP and **redone**, never skipped - skipping silently drops the work the crash interrupted while still looking like a clean resume |
